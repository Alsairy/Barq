...
public class SecurityMonitoringService : ISecurityMonitoringService
{
    ...
    public Task<IEnumerable<SecurityEventDto>> GetSecurityEventsAsync(DateTime? fromDate = null, DateTime? toDate = null, string? eventType = null)
    {
        try
        {
            ...
            return Task.CompletedTask.ContinueWith(_ => (IEnumerable<SecurityEventDto>)query.OrderByDescending(e => e.Timestamp).ToList());
        }
        catch (Exception ex)
        {
    ...
    public Task<IEnumerable<SecurityAlertDto>> GetActiveAlertsAsync()
    {
        try
        {
            return Task.CompletedTask.ContinueWith(_ => (IEnumerable<SecurityAlertDto>)_activeAlerts.Where(a => a.IsActive).OrderByDescending(a => a.CreatedAt));
        }
        catch (Exception ex)
        {
